      ...
      <div className=" flex justify-between items-center w-full h-14 px-6 max-w-[1200px]">
        <div className="w-4/5 px-1.5 flex justify-start items-center no-underline max-[640px]:p-0 ">
          <Link href="/" className="flex items-center text-white cursor-pointer">
            <DiCssdeck size="3rem" /> <span className="px-1 font-bold text-lg">Portfolio</span>
          </Link>
        </div>
        ...
        <div className="w-4/5 h-full flex justify-end items-center px-1.5 max-[768px]:hidden">
          <Link href={Bio.github} target="_blank" className="flex items-center justify-center h-[70%] rounded-[20px] border border-[#854ce6] text-[#854ce6] cursor-pointer px-[20px] font-medium text-[16px] transition-all duration-600 ease-in-out hover:bg-purple-600 hover:text-white max-[768px]:text-[14px] ">
            Github Profile
          </Link>
        </div>
        ...
        <div className="hidden max-[768px]:block">
          {isOpen && (
            <div className="bg-[#191924] flex flex-col justify-center items-center gap-4 absolute top-20 right-0 w-full py-3 px-10 transition-all duration-200 ease-in-out text-white rounded-b-lg shadow-md opacity-100 z-50">
              <Link href="#about" className="text-[#F2F3F4] font-medium cursor-pointer transition-all duration-200 ease-in-out no-underline hover:text-purple-600 active:border-b-2 border-purple-600"
                onClick={() => setIsOpen(!isOpen)}
              > About </Link>

              <Link href="#skills" className="text-[#F2F3F4] font-medium cursor-pointer transition-all duration-200 ease-in-out no-underline hover:text-purple-600 active:border-b-2 border-purple-600"
                onClick={() => setIsOpen(!isOpen)}
              > Skills </Link>

              <Link href="#experience" className="text-[#F2F3F4] font-medium cursor-pointer transition-all duration-200 ease-in-out no-underline hover:text-purple-600 active:border-b-2 border-purple-600"
                onClick={() => setIsOpen(!isOpen)}
              > Experience </Link>
              ...
              <Link href={Bio.github} className=" rounded-full border border-[#854ce6] text-[#854ce6] cursor-pointer px-5 py-2.5 font-medium  transition-all duration-200 ease-in-out no-underline" target="_blank">
                Github Profile
              </Link>
            </div>
          )}
